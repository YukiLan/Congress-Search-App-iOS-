//
//  FavCommiDetailViewController.swift
//  hw9
//
...
class FavCommiDetailViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    @IBAction func addFav(_ sender: Any) {
        if (self.detail["chamber"].stringValue == "house"){
            if (CommiHouseDetailViewController.favList.contains(self.detail)){
                self.fav.title = String.fontAwesomeIcon(name: .starO)
                CommiHouseDetailViewController.favList = CommiHouseDetailViewController.favList.filter{$0 != self.detail}
            }
            else{
                self.fav.title = String.fontAwesomeIcon(name: .star)
                CommiHouseDetailViewController.favList.append(self.detail)
            }
        }
            ...
            if (CommiJointDetailViewController.favList.contains(self.detail)){
                self.fav.title = String.fontAwesomeIcon(name: .starO)
                CommiJointDetailViewController.favList = CommiJointDetailViewController.favList.filter{$0 != self.detail}
            }
            else{
                self.fav.title = String.fontAwesomeIcon(name: .star)
                CommiJointDetailViewController.favList.append(self.detail)
            }
        }
