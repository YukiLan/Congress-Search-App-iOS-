//
//  FavLegisDetailViewController.swift
//  hw9
//
...
class FavLegisDetailViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    @IBAction func addFav(_ sender: Any) {
        ...
        if (self.detail["chamber"].stringValue == "house"){
            if (LegisHouseDetailViewController.favList.contains(self.detail)){
                
                self.fav.title = String.fontAwesomeIcon(name: .starO)
                
                LegisHouseDetailViewController.favList = LegisHouseDetailViewController.favList.filter{$0 != self.detail}
            }
            else{
                self.fav.title = String.fontAwesomeIcon(name: .star)
                LegisHouseDetailViewController.favList.append(self.detail)
            }
        }
        else if (self.detail["chamber"].stringValue == "senate"){
            if (LegisSenateDetailViewController.favList.contains(self.detail)){
                self.fav.title = String.fontAwesomeIcon(name: .starO)
                
                LegisSenateDetailViewController.favList = LegisSenateDetailViewController.favList.filter{$0 != self.detail}
            }
            else{
                self.fav.title = String.fontAwesomeIcon(name: .star)
                LegisSenateDetailViewController.favList.append(self.detail)
            }
        }
        else{

            if (LegisDetailViewController.favList.contains(self.detail)){
                print(1)
                self.fav.title = String.fontAwesomeIcon(name: .starO)
                
                LegisDetailViewController.favList = LegisDetailViewController.favList.filter{$0 != self.detail}
            }
            else{
                print(2)
                self.fav.title = String.fontAwesomeIcon(name: .star)
                LegisDetailViewController.favList.append(self.detail)
            }
        }
*/
